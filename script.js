...
function getTemp() {
  ...
  fetch(url)
    ...
    .then(location => {
      ...
      document.querySelector("h3").innerText = `Temperature is ${ctof}°, but feels: ${ctof2}°`
      document.querySelector("h4").innerText = `Precipitation is ${location.current.precip}, and humidity is ${location.current.humidity} `
      // document.querySelector("h5").innerText = `Feels like: ${ctof2}`
      // document.querySelector("h6").innerText = `Humidity: ${location.current.humidity}`
    })
    .catch(err => {
